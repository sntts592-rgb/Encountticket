...
function initializeKB() {
  try {
    ...
    let importedData: any = ticketsData;
    if (importedData && !Array.isArray(importedData) && Array.isArray((importedData as any).default)) {
      importedData = (importedData as any).default;
    }

    if (Array.isArray(importedData) && importedData.length > 0) {
      console.log(`[Vercel Serverless] Loading directly from bundled JSON module: ${importedData.length} records.`);
      kbRecords = importedData as KBRecord[];
      return;
    }
  } catch (err) {
